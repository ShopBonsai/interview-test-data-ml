...
Note that your model should be feasible in a production environment - I.E. a complex, deeplearning model might outperform simpler models in terms of recommendation results, but it will be very slow to train. In production, a recommender system will often make batch recommendations for all our current users, and should therefore be fast enough to warrant use. Aim to present a good balance between speed and evaluation metrics (see below). 
